...
/**
 * Compatibility note: 使用 ViewInternalProperties 代替
 * 保留此别名以兼容现有代码
 */
 ...
 * 提供对编辑器和内部属性的访问
 */
export interface ExtendedMarkdownView
	extends Omit<MarkdownView, "editor" | "contentEl" | "file" | "currentMode"> {
	/** CodeMirror 编辑器实例 */
...
export interface ExtendedEditor
	extends Omit<Editor, "setValue" | "getValue" | "refresh" | "focus"> {
	/** CodeMirror 实例 */
 ...
 */
export function isExtendedWorkspaceLeaf(leaf: unknown): leaf is ExtendedWorkspaceLeaf {
	return typeof leaf === "object" && leaf !== null && "view" in leaf && "containerEl" in leaf;
}

/**
 * 类型守卫：检查是否为扩展的 MarkdownView
 */
export function isExtendedMarkdownView(view: unknown): view is ExtendedMarkdownView {
	return typeof view === "object" && view !== null && "editor" in view && "file" in view;
}

/**
 * 类型守卫：检查是否为扩展的 Editor
 */
export function isExtendedEditor(editor: unknown): editor is ExtendedEditor {
	return (
		typeof editor === "object" && editor !== null && ("setValue" in editor || "getValue" in editor)
	);
}
 ...
 * 安全获取 Leaf 容器元素
 */
export function getLeafContainerElement(leaf: WorkspaceLeaf): HTMLElement | null {
 ...
 * 安全设置编辑器内容
 */
export function safeSetEditorValue(editor: Editor, content: string): boolean {
	try {
		const extendedEditor = editor as ExtendedEditor;
 ...
 * 安全获取编辑器内容
 */
export function safeGetEditorValue(editor: Editor): string | null {
	try {
		const extendedEditor = editor as ExtendedEditor;
